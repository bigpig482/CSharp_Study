...
namespace Lesson19_多态_接口
{
    ...
    //那么接口也是遵循里氏替换原则，所以可以用接口容器装对象
    //那么就可以实现 装载各种毫无关系但是却有相同行为的对象

    //注意：
    //1.接口值包含 成员方法、属性、索引器、事件，并且都不实现，都不能是私有的
    //2.可以继承多个接口,但是只能继承一个类
    //3.接口可以继承接口，相当于在进行行为合并，待子类继承事再去实现具体的行为
    //4.接口可以被显示实现 主要作用与实现不同接口中的同名函数的不同表现
